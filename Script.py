...
st.set_page_config(
    page_title="Simulador RLC Completo e Interativo",
    page_icon="⚡",
    layout="wide",
)
...
st.markdown(
    ...
    "corrente, potências, diagramas fasoriais e triângulo de potências em tempo real."
)
...
# --- MÓDULO VISUAL: DRAG & DROP (OPCIONAL DE BANCADA) ---
with st.expander("🖐️ Bancada Interativa de Arraste (Drag & Drop)", expanded=False):
    st.write("Arraste os componentes pré-definidos para visualizar a alocação nos slots do circuito.")
    drag_drop_html = """
    <style>
...
if not st.session_state.componentes:
    st.warning("Nenhum componente adicionado. Utilize o menu lateral para adicionar elementos.")
    st.stop()
...
for idx, comp in enumerate(st.session_state.componentes):
    ...
    with cols[col_idx]:
        st.metric(f"{idx+1}. {comp['tipo']}", f"{comp['valor']} {comp['unidade']}")
        if st.button("🗑️ Remover", key=f"del_{idx}"):
            st.session_state.componentes.pop(idx)
            st.rerun()

# --- FUNÇÕES DE CÁLCULO ELETRÓNICO ---
...
try:
    if arranjo == "Série Puro":
        Z_eq = sum(calcular_impedancia_item(c, w) for c in st.session_state.componentes)
    elif arranjo == "Paralelo Puro":
        Y_total = sum(1 / calcular_impedancia_item(c, w) for c in st.session_state.componentes)
        Z_eq = 1 / Y_total
    elif arranjo == "Misto (Resistor em Série + Bloco Paralelo)":
        res_list = [c for c in st.session_state.componentes if c["tipo"] == "Resistor"]
        reat_list = [c for c in st.session_state.componentes if c["tipo"] != "Resistor"]
        if not res_list or not reat_list:
            st.error("Para o arranjo Misto, adicione pelo menos 1 Resistor e 1 elemento reativo (L ou C).")
            st.stop()
...
def desenhar_esquematico(componentes, modo_arranjo, tensao):
    ...
    ax_esq.add_patch(patches.Circle((-1, 2), 0.4, fill=False, color="red", lw=2))
    ax_esq.text(-1, 2, "~", fontsize=18, ha="center", va="center", color="red", weight="bold")
    ...
    if modo_arranjo == "Série Puro":
        ...
        for comp in componentes:
            ...
            rect = patches.Rectangle((x_curr + x_step * 0.15, 3.6), x_step * 0.7, 0.8, facecolor="whitesmoke", edgecolor="navy", lw=2)
            ax_esq.add_patch(rect)
            ax_esq.text(x_curr + x_step * 0.5, 4, f"{comp['tipo'][0]}:{comp['valor']}{comp['unidade']}", fontsize=8, ha="center", va="center", weight="bold")
            x_curr = x_next
        ...
        for i, comp in enumerate(componentes):
            ...
            rect = patches.Rectangle((x_pos - 0.4, 1.5), 0.8, 1.0, facecolor="whitesmoke", edgecolor="darkgreen", lw=2)
            ax_esq.add_patch(rect)
            ax_esq.text(x_pos, 2.0, f"{comp['tipo'][0]}\n{comp['valor']}{comp['unidade']}", fontsize=8, ha="center", va="center", weight="bold")
        ...
        rect_r = patches.Rectangle((0.5, 3.6), 1.0, 0.8, facecolor="whitesmoke", edgecolor="darkorange", lw=2)
        ax_esq.add_patch(rect_r)
        lbl_r = f"R:{res_list[0]['valor']}Ω" if res_list else "R_série"
        ax_esq.text(1.0, 4.0, lbl_r, fontsize=8, ha="center", va="center", weight="bold")
        ...
        for i, comp in enumerate(reat_list):
            ...
            ax_esq.plot([x_pos, x_pos], [4, 2.5], color="black", lw=2)
            ax_esq.plot([x_pos, x_pos], [1.5, 0], color="black", lw=2)
            rect = patches.Rectangle((x_pos - 0.4, 1.5), 0.8, 1.0, facecolor="whitesmoke", edgecolor="purple", lw=2)
            ax_esq.add_patch(rect)
            ax_esq.text(x_pos, 2.0, f"{comp['tipo'][0]}\n{comp['valor']}{comp['unidade']}", fontsize=8, ha="center", va="center", weight="bold")
...
with r2:
    ...
    carater = "Indutivo" if Q > 0.01 else "Capacitivo" if Q < -0.01 else "Resistivo"
...
with g1:
    ...
    ax_f.quiver(0, 0, u_I, v_I, angles="xy", scale_units="xy", scale=1, color="blue", label=f"I = {I_rms:.2f}A ∠{angle_I_deg:.1f}° (x{escala_I:.0f})")
